...
NOTE!!!!!! centerness = centerness.permute(1, 2, 0).reshape(-1).sigmoid() should be optimized, if disabeld centerness
"""
import torch
...
class RotatedFCOSHead_ST(RotatedFCOSHead):
    def __init__(self,
                 num_classes,
                 in_channels,
    ...
    def _get_bboxes_single(self,
        ...
        for idx, data_level in enumerate(zip(
            ...
            if not self.disable_centerness:
                # as we assign centerness as one when not used, .sigmoid should not be used.
                centerness = centerness.permute(1, 2, 0).reshape(-1).sigmoid()
            else: centerness = centerness.permute(1, 2, 0).reshape(-1)
