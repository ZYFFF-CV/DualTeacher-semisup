...
# modified from https://github.com/liuyanyi/mmrotate/blob/ryolox/mmrotate/models/dense_heads/rotated_yolox_head.py
# exclude non-positive in the bbox loss
import numpy as np
import torch
...
class RotatedYOLOXHead_ST(YOLOXHead):
    ...
    def loss(self,
        ...
        if num_pos > 0: # Avoiding No label
            # Loss Bbox
            if self.separate_angle:
            ...
            # Loss Bbox
            if self.separate_angle:
                loss_bbox = flatten_hbboxes.view(-1, 4)[pos_masks].sum()
                loss_angle = flatten_angle_preds.view(-1, self.angle_len)[pos_masks].sum()
            else:
                loss_bbox = flatten_rbboxes.view(-1, 5)[pos_masks].sum()
            # Loss Cls   
            loss_cls = flatten_cls_preds.view(-1, self.num_classes)[pos_masks].sum()
                ...
                if self.with_angle_l1:
                    flatten_rbbox_preds = torch.cat([
                        ...
                        flatten_decoded_angle / self.angle_norm_factor
                                                    ...
                                                    dim=-1)
                    loss_l1 = flatten_rbbox_preds.view(-1, 5)[pos_masks].sum()
                else:
                    loss_l1 = flatten_bbox_preds.view(-1, 4)[pos_masks].sum()
    ...
    def _get_target_single(self, cls_preds, objectness, priors, decoded_bboxes,
        ...
        return super(RotatedYOLOXHead_ST,
