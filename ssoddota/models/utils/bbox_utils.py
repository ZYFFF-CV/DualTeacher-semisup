"""Two main updates
1. The original SoftTeacher only uses 'Transform2D' and 'filter_invalid'. Thus, we only keep those two for convinence
2. Mmdet uses 'xyxy' format furing training, while mmrot uses 'xywha'. 
Thus, we convert the origianl format to fit this requirement
"""
import warnings
...
from mmrotate.core import norm_angle, obb2poly, poly2obb
...
class Transform2D:
    @staticmethod
    def transform_rbboxes(bbox, M, out_shape, version):
        """Specified for mmrot bbox in xywha format
        """
        if isinstance(bbox, Sequence):
            assert len(bbox) == len(M)
            return [
                Transform2D.transform_rbboxes(b, m, o, version)
            ...
            points = obb2poly(bbox[:, :5],version).reshape(-1, 2)  # n*4,2
            ...
            if points.size()[0] > 0:
                bbox = poly2obb(points, version) #xywh  n,4
                
                bbox[:,0] = bbox[:,0].clamp(min=0, max=out_shape[1])
                bbox[:,1] = bbox[:,1].clamp(min=0, max=out_shape[0])
                bbox[:,2] = bbox[:,2].clamp(min=0, max=out_shape[1])
                bbox[:,3] = bbox[:,3].clamp(min=0, max=out_shape[0])
      
            else:
                bbox = bbox.new_zeros(0, 5)
...
def filter_invalid_mmrot(bbox, label=None, score=None, mask=None, thr=0.0, min_size=0):
    """
    BBox in mmrot is whwya
    """
